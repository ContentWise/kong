...
local policy_cluster = require "kong.plugins.rate-limiting-failover.policies.cluster"
...
local get_local_key = function(api_id, identifier, period_date, name)
  return fmt("ratelimit-failover:%s:%s:%s:%s", api_id, identifier, period_date, name)
end
...
return {
    ...
    increment = function(conf, limits, api_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          ...
          if err then
            ngx_log("[rate-limiting-failover] could not increment counter for period '" .. period .. "': " .. tostring(err))
            return nil, err
          end
        end
      end
  ...
  ["cluster"] = {
    increment = function(conf, limits, api_id, identifier, current_timestamp, value)
      ...
      if not ok then
        ngx_log(ngx.ERR, "[rate-limiting-failover] cluster policy: could not increment ",
                          db.name, " counter: ", err)
      end
