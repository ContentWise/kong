  ...
  ["cassandra"] = {
    increment = function(db, limits, api_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          ...
          if not res then
            log(ERR, "[rate-limiting-failover] cluster policy: could not increment ",
    ...
    find = function(db, api_id, identifier, current_timestamp, period)
      ...
      local rows, err = db:query([[
        SELECT *
        FROM ratelimiting_failover_metrics
        WHERE api_id = ? AND
              identifier = ? AND
              period_date = ? AND
              period = ?
  ...
  ["postgres"] = {
    increment = function(db, limits, api_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          buf[#buf+1] = fmt([[
            SELECT increment_rate_limits_failover('%s', '%s', '%s', to_timestamp('%s')
            at time zone 'UTC', %d)
