return {
  {
    name = "2015-08-03-132400_init_ratelimiting_failover",
    up = [[
      ...
      CREATE OR REPLACE FUNCTION increment_rate_limits_failover(a_id uuid, i text, p text, p_date timestamp with time zone, v integer) RETURNS VOID AS $$
      BEGIN
        LOOP
          UPDATE ratelimiting_failover_metrics SET value = value + v WHERE api_id = a_id AND identifier = i AND period = p AND period_date = p_date;
          IF found then
            RETURN;
          END IF;

          BEGIN
            INSERT INTO ratelimiting_failover_metrics(api_id, period, period_date, identifier, value) VALUES(a_id, p, p_date, i, v);
            RETURN;
  ...
  {
    name = "2016-07-25-471385_ratelimiting_failover_policies",
    up = function(_, _, dao)
